...
def kl_test(df_in: dataframe, df_out: dataframe, attr: str):
    ...
    """
    distribution_in: dataframe.Series = get_distribution_of_categorical_attribute(df_in[attr])
...
class ModelInspector(object):
    def __init__(self, private_df: dataframe, synthetic_df: dataframe, attribute_description):
        ...
        self.attribute_description = attribute_description['attribute_description']
        ...
        self.private_df.drop(columns='business_id')
        self.synthetic_df.drop(columns='business_id')
        #self.synthetic_df.drop(columns=list(self.candidate_keys))
